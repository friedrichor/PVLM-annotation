...
class Trainer(object):  # 这里没有保存模型文件的代码
    ...
    def save(self):  # 保存测试集结果, 
        ...
        with open(os.path.join(self.save_dir, ckpt_name) + '.txt', 'w', encoding='utf-8') as f:
            ...
            for y, y_ in zip(test_y, test_y_):
                token_y = self.test_set.tokenizer.convert_ids_to_tokens(y)  # 真实结果
                token_y_ = self.test_set.tokenizer.convert_ids_to_tokens(y_)  # 预测结果
    ...
    def load(self):
        ...
        self.model.load_state_dict(ckpt_dict["embedding"])  # embedding存储的是模型参数

    def _evaluate(self, evaluate_type):
        self.model.eval()
        if evaluate_type == 'Test':  # 选取数据集, 验证集/测试集
    ...
    def update_best_ckpt(self, epoch_idx=None, dev_acc=None, test_acc=None, test_f1=None, test_y=None, test_y_=None):
        if test_acc == None:  # during training  训练时保存参数
            model_params = copy.deepcopy(self.model.state_dict())
            self.best_ckpt['time'] = datetime.now()
            self.best_ckpt['embedding'] = model_params  # best_ckpt['embedding'] -- 模型参数
    ...
    def train(self):
        ...
        params = []
        params.append({'params': self.model.lm_model.parameters(), 'lr': self.args.lr_lm_model})  # lm_model--BertForMaskedLM
        if self.args.template == 3:
        ...
        if not self.args.no_img:
            ...
            # backbone: VISUAL_MODELS = ['nf_resnet50', 'resnet50', 'resnetv2_50x1_bitm', 'vit_base_patch16_224']
        ...
        for epoch_idx in range(100):
            ...
            pbar = tqdm(self.train_loader, unit="batch")  # 描述处理项目的文字，默认’it’，即10it/s；这里设置为’batch’，则为10batch/s
            for batch in pbar:
                self.model.train()
                input_ids = batch['input_ids'].to(self.device)
                attention_mask = batch['attention_mask'].to(self.device)
                ...
                _loss, _y_, _y = self.model(input_ids, attention_mask, labels, imgs)  # 损失, 预测的情感标签, 真实的情感标签(vocab中的索引)
                loss.append(_loss.item())
                y_.extend(_y_)
                y.extend(_y)
